  ...
  const deleteData = () => {
    updateSettings({
      dataPrivacy: {
        ...
        downloadData: settings?.dataPrivacy?.downloadData ?? false,
        exportData: settings?.dataPrivacy?.exportData ?? false,
        deleteData: settings?.dataPrivacy?.deleteData ?? false,
        activityControls: {
          postHistory: false,
          searchHistory: false,
          interactionHistory: false
        },
        dataPermissions: settings?.dataPrivacy?.dataPermissions || { analytics: true, recommendations: true, aiTraining: false },
        cookies: settings?.dataPrivacy?.cookies || { essential: true, analytics: false, personalization: false }
      }
          ...
          {activeSection === 'analytics-export' && (
            <div className="settings-section">
              ...
              <div className="settings-card">
                ...
                {[
                  { key: 'pdf' as 'pdf' | 'csv' | 'json', label: 'PDF' },
                  { key: 'csv' as 'pdf' | 'csv' | 'json', label: 'CSV' },
                  { key: 'json' as 'pdf' | 'csv' | 'json', label: 'JSON' }
                ].map(format => (
