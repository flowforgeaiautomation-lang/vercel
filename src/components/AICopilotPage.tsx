...
const AICopilotPage: React.FC = () => {
  ...
  const archProfile = userData?.architectProfile as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const catProfile = userData?.catalystProfile as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const expProfile = userData?.explorerProfile as any;

  const startupInfo = archProfile ? {
    companyName: archProfile.companyName,
    companyDescription: archProfile.companyDescription,
    startupStage: archProfile.startupStage,
    industry: archProfile.industry,
    teamSize: archProfile.teamSize,
    fundingStage: archProfile.fundingStage,
    goals: archProfile.goals,
    challenges: archProfile.challenges,
  } : null;

  const investmentInfo = catProfile ? {
    investorType: catProfile.investorType,
    focusAreas: catProfile.focusAreas,
    stages: catProfile.stages,
    investmentSize: catProfile.investmentSize,
    checkSize: catProfile.checkSize,
    portfolioCompanies: catProfile.portfolioCompanies,
    activeDeals: catProfile.activeDeals,
    interests: catProfile.interests,
  } : null;

  const explorerInfo = expProfile ? {
    skills: expProfile.skills,
    interests: expProfile.interests,
    experience: expProfile.experience,
    collaboration: expProfile.collaboration,
    availability: expProfile.availability,
    goals: expProfile.goals,
  } : null;
