...
interface PostContextType {
  ...
  signals: Signal[];
  addPost: (post: Omit<Post, 'id' | 'likes' | 'likedBy' | 'comments' | 'shares' | 'timestamp' | 'engagementScore' | 'isSaved'>) => void;
...
export const PostProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const likePost = (postId: string) => {
    ...
    const currentUserAvatar = userData?.profile?.profileImage;
  ...
  const repost = (originalPostId: string, comment?: string) => {
    ...
    const newRepost: Repost = {
      ...
      userAvatar: user?.profile?.profileImage,
      userRole: user?.mainRole || 'EXPLORER',
      comment,
      createdAt: new Date()
    };
  ...
  const publishScheduledPost = (postId: string) => {
    ...
    if (scheduled) {
      addPost({
        ...
        userUsername: userData?.username || '',
        userAvatar: userData?.profile?.profileImage,
        userRole: userData?.mainRole || 'EXPLORER',
        userTitle: userData?.profile?.title,
