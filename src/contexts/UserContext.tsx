...
interface VerificationData {
  ...
  status: 'unverified' | 'pending' | 'ready_for_approval' | 'approved' | 'rejected' | 'needs_action' | 'flagged_for_review';
...
interface UserContextType {
  ...
  aiVerificationEngine: Record<string, any>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  riskEngine: Record<string, any>;
}
...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    if (user && authProfile) {
      ...
      // Check Firestore for existing data in background (with safe error handling)
          ...
          getDoc(userRef).then((docSnap) => {
            ...
            // Run anti-fraud check and trust index calculation in background
            runAntiFraudCheck(user.uid, finalData).catch(() => {});
            calculateAndSaveTrustIndex(user.uid, finalData).catch(() => {});
          }).catch((error) => {
  ...
  const calculateVerificationLevel = (trustScore: number): 'unverified' | 'Basic Verified' | 'Identity Verified' | 'Professional Verified' | 'TRIVEON Verified' => {
  ...
  const followUser = async (userIdToFollow: string) => {
    if (!userData || userIdToFollow === userData.uid) return;
    // Call real Firebase follow (creates follows doc, increments counters, sends signal)
    await fbFollowUser(userData.uid, userIdToFollow);
    // Also keep local following array in sync
    ...
    // Recalculate trust index after follow action
    if (userData.uid) {
      calculateAndSaveTrustIndex(userData.uid, userData).catch(() => {});
    }
  };

  const unfollowUser = async (userIdToUnfollow: string) => {
    if (!userData) return;
    // Call real Firebase unfollow (deletes follows doc, decrements counters)
    await fbUnfollowUser(userData.uid, userIdToUnfollow);
